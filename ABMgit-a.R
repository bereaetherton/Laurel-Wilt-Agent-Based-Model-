...
setwd(dir = "../../git/Laurel-Wilt-Agent-Based-Model-/")
...
beta.bp <- 2.0  #beta value for biophysical network
...
avocado.groves <- avocado.groves %>% 
                    mutate(disease.presence = sample( c(0,1), size, replace = TRUE, prob=c((1-pod.initial), pod.initial)))%>%
                    mutate(diseased.trees=0)%>%
